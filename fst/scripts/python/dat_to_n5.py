...
def prepare_data(path: Union[str, list]) -> Tuple[list, list]:
    ...
    if isinstance(path, str):
        ...
        raise ValueError(
            f"Path variable should be string or list, not {type(path)}")
    ...
    data = arrays_from_delayed(
        read(fnames, lazy=True), shapes=shapes, dtypes=dtypes)
    return data, meta
...
def prepare_pyramids(data, fill_values, chunks, reduction, downscale_factor, output_dtype) -> Tuple[list, list]:
    padded, padding = pad_arrays(data, constant_values=fill_values)
    stacked = da.stack(padded).rechunk(chunks)
    if output_dtype != 'same':
        stacked = (stacked - fill_values.reshape(1, -1, 1, 1)
                   ).astype(output_dtype)
    scaling_factors = (1, 1, downscale_factor, downscale_factor)
    pyramid = lazy_pyramid(
        stacked, reduction, scaling_factors, preserve_dtype=True)
...
def dat_to_n5(source: str, dest: str, num_workers: Union[str, int], multiscale_levels: int, datatype: str, output_chunks: tuple):
    ...
    if np.any(np.isnan(minmax)):
        ...
        logger.info(
            "Calculating minimum and maximum values of the input data...")
    ...
    logger.info(
        f"Assembled dataset with shape {(len(padded), *padded[0].shape)}"
    )
            ...
            array_attrs.append(
                {
                    ...
                    "pixelResolution": {
                        "dimensions": np.multiply(grid_spacing_n5, scales[ind_l]).tolist(),
    ...
    to_store = [save_pyramid_slicewise(arr=sl,
                                       z_index=z_idx,
                                       scale_factors=(
...
def main():
    ...
    return dat_to_n5(source=args.source,
                     dest=args.dest,
