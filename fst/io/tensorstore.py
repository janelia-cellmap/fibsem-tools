...
class TensorStoreArray():
        def __init__(self,
            ...
            scale_metadata = ImageScaleMetadata(chunk_size=chunk_size, 
                                           size=size,
                                           voxel_offset=offset, 
            ...
            kvstore = KVStore(driver=kvstore_driver, 
                              path=kvstore_path)

            multiscale_metadata = MultiscaleMetadata(data_type=dtype,
                                                     num_channels=num_channels,
                                                     type=volume_type)

            self.spec = asdict(TensorStoreSpec(driver, 
                                   kvstore, 
                                   path, 
                                   scale_metadata, 
                                   multiscale_metadata))
