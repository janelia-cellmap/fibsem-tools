...
def prepare_experiment_dicts(rootdir):
    ...
    mask_dicts = [
        {"needs_mask": needs_mask[ind], "mask_threshold": mask_threshold[ind]}
...
if __name__ == "__main__":
    rootdir = (
        "/groups/hess/hess_collaborators/Annotations/ParentFiles_whole-cell_images/"
    )
    ...
    if exists(metadata_fname):
        if overwrite == True:
            ...
            print(
                f"File {metadata_fname} exists. Set `overwrite=True` to overwrite it."
            )

    # Write YAML file
    with io.open(metadata_fname, "w", encoding="utf8") as outfile:
        yaml.dump(
            data, outfile, default_flow_style=False, allow_unicode=True, sort_keys=False
        )
