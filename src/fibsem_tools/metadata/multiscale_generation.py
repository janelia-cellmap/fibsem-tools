...
class WriteableArrayStore(ArrayStore):
    access_mode: Union[WriteableAccessMode, Tuple[WriteableAccessMode, WriteableAccessMode]]
...
def prepare_multiscale_storage(source: str, 
                      source_chunks: Union[Tuple[int, ...], ChunkMode],
                      dest: str,
                      dest_chunks: Union[Tuple[int, ...], ChunkMode],
                      dest_access_mode: Union[WriteableAccessMode, Tuple[WriteableAccessMode, WriteableAccessMode]],
    ...
    if downsampling_method == 'mean':
        ...
        raise ValueError(f'Invalid downsampling method. Must be one of ("mean", "mode"), got {downsampling_method}')
...
def main(config_json: str, 
    ...
    logger.addHandler(logging.FileHandler(filename=os.path.join(spec.logging_dir, f'multiscale_generation_{now_str}.log')))
    logger.addHandler(logging.StreamHandler())
    logger.info(f'Loaded MultiscaleStorageSpec: {spec.json(indent=2)}')

    store_op = prepare_multiscale_storage(source = spec.source.url,
    ...
    if spec.cluster_spec.deployment == 'dask_local':
        ...
        clusterClass = partial(LSFCluster, 
    ...
    if not dry:
        logger.info(f'Creating an instance of {clusterClass.__name__} and scaling to {spec.cluster_spec.worker.num_workers} workers')
        with clusterClass() as clust, Client(clust) as cl, performance_report(os.path.join(spec.logging_dir, f'dask_distributed_perf_report_{now_str}.html')):
