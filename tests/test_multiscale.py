...
def test_multiscale_storage(multiscale_metadata: bool,
                            propagate_array_attrs: bool):
    data = da.random.randint(0, 8, (16, 16), chunks=(8, 8), dtype='uint8')
    coords = (('x', da.arange(data.shape[0]), {'units': 'nm'}),
    ...
    for idx, k in enumerate(multi):
        ...
        if multiscale_metadata:
            cosem_meta = COSEMGroupMetadata.fromDataArrays(name=ms.name,
                                                           paths=tuple(multi.keys()),
                                                           dataarrays=tuple(multi.values())).dict()
